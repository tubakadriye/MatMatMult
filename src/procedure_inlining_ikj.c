				...
				for ( k=0; k < block_size ; k++)
				{
					...
					for(p=0; p < block_size; p+=13)
					{
						...
						for (m=0; m < block_size ; m++)
						{
							...
							C[i*block_size*n + j*block_size + k*n + m] += A[i*block_size*n + (l+1)*block_size + k*n +p+1] *
									B[j*block_size*n + (l+1) *block_size + m*n +p+1];
							C[i*block_size*n + j*block_size + k*n + m] += A[i*block_size*n + (l+2)*block_size + k*n +p+2] *
									B[j*block_size*n + (l+2) *block_size + m*n +p+2];
							C[i*block_size*n + j*block_size + k*n + m] += A[i*block_size*n + (l+3)*block_size + k*n +p+3] *
									B[j*block_size*n + (l+3) *block_size + m*n +p+3];
							C[i*block_size*n + j*block_size + k*n + m] += A[i*block_size*n + (l+4)*block_size + k*n +p+4] *
									B[j*block_size*n + (l+4) *block_size + m*n +p+4];
							C[i*block_size*n + j*block_size + k*n + m] += A[i*block_size*n + (l+5)*block_size + k*n +p+5] *
									B[j*block_size*n + (l+5) *block_size + m*n +p+5];
							C[i*block_size*n + j*block_size + k*n + m] += A[i*block_size*n + (l+6)*block_size + k*n +p+6] *
									B[j*block_size*n + (l+6) *block_size + m*n +p+6];
							C[i*block_size*n + j*block_size + k*n + m] += A[i*block_size*n + (l+7)*block_size + k*n +p+7] *
									B[j*block_size*n + (l+7) *block_size + m*n +p+7];
							C[i*block_size*n + j*block_size + k*n + m] += A[i*block_size*n + (l+8)*block_size + k*n +p+8] *
										B[j*block_size*n + (l+8) *block_size + m*n +p+8];
							C[i*block_size*n + j*block_size + k*n + m] += A[i*block_size*n + (l+9)*block_size + k*n +p+9] *
										B[j*block_size*n + (l+9) *block_size + m*n +p+9];
							C[i*block_size*n + j*block_size + k*n + m] += A[i*block_size*n + (l+10)*block_size + k*n +p+10] *
										B[j*block_size*n + (l+10) *block_size + m*n +p+10];
							C[i*block_size*n + j*block_size + k*n + m] += A[i*block_size*n + (l+11)*block_size + k*n +p+11] *
										B[j*block_size*n + (l+11) *block_size + m*n +p+11];
							C[i*block_size*n + j*block_size + k*n + m] += A[i*block_size*n + (l+12)*block_size + k*n +p+12] *
